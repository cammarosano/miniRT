...
/*   Updated: 2021/04/07 17:47:11 by rcammaro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
						...
						t_vect spot_adj_color, t_vect nd_view_ray)
{
	...
	result = v_scale(0.15 * log(hp->shine), result);
	return (result);
}

static t_vect	get_shadow_ray(t_light *light, t_vect point, double *dist)
{
	if (light->type == spot)
	{
		*dist = 1.0;
		return (v_subtract(light->pos_dir, point));
	}
	*dist = 1.0 / 0.0;
	return (v_scale(-1, light->pos_dir));
}
...
t_vect	cast_shadow_ray(t_light *lightspot, t_hit_point *hp,
								t_list *obj_list, t_vect view_ray)
{
	...
	shadow_ray = get_shadow_ray(lightspot, hp->point, &dist);
	if (v_lenght(shadow_ray) < EPSILON)
		return (vector(0, 0, 0));
	while (obj_list)
	{
		obj = obj_list->content;
		if (intercept_obj(hp->point, shadow_ray, obj, EPSILON) < dist)
			return (vector(0, 0, 0));
