...
/*   Updated: 2021/04/07 18:02:48 by rcammaro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	int			index;
	static	int	(*func[11])(char **, t_scene *, int) = {parse_resolution,
	parse_ambient_light, parse_camera, parse_lightspot, parse_sphere,
	parse_plane, parse_square, parse_triangle, parse_cylinder, parse_cube,
	parse_pyramid};

	index = word_to_index(words[0], "R A c l sp pl sq tr cy cu py");
	if (index == -1)
